...
Also you can make a backups of you group configuration, so that you don't have to worry about messing it up. Backups explained [here](#save-file). 
...
Select the button within the build tab (not within the group) and use that arrows.
...
One **important** thing is that Object Groups restricts the removal of buttons from the tabs where they located in vanilla GD. (This is mainly made in order to prevent the loss of objects after their removal from groups) 
...
It tries to put the selected objects in a new group while preserving their relative positions. To do this, the objects must be arranged in a grid-like manner in the editor (there must be a distinct rows and columns with enough spaces between them. Alignment to the editor grid is not necessary). Example:
