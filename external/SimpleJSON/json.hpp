...
class JSON
{
    ...
    public:
        ...
        static JSON Load( const string &, std::vector<std::string> * const );
...
namespace {
    JSON parse_next( const string &, size_t &, std::vector<std::string> * const);

    void consume_ws( const string &str, size_t &offset ) {
        while( isspace( str[offset] ) ) ++offset;
    }

    JSON parse_object( const string &str, size_t &offset, std::vector<std::string> * const err_list ) {
        ...
        while( true ) {
            ...
            consume_ws( str, ++offset );
            JSON Value = parse_next( str, offset, err_list );
    ...
    JSON parse_array( const string &str, size_t &offset, std::vector<std::string> * const err_list ) {
        ...
        while( true ) {
            Array[index++] = parse_next( str, offset, err_list );
            consume_ws( str, offset );
    ...
    JSON parse_string( const string &str, size_t &offset, std::vector<std::string> * const err_list ) {
        JSON String;
        string val;
    ...
    JSON parse_number( const string &str, size_t &offset, std::vector<std::string> * const err_list ) {
        JSON Number;
        string val, exp_str;
        char c;
        bool isDouble = false;
    ...
    JSON parse_bool( const string &str, size_t &offset, std::vector<std::string> * const err_list ) {
        JSON Bool;
    ...
    JSON parse_next( const string &str, size_t &offset, std::vector<std::string> * const err_list ) {
        char value;
        consume_ws( str, offset );
        value = str[offset];
        switch( value ) {
            case '[' : return std::move( parse_array( str, offset, err_list ) );
            case '{' : return std::move( parse_object( str, offset, err_list ) );
            case '\"': return std::move( parse_string( str, offset, err_list ) );
            case 't' :
            case 'f' : return std::move( parse_bool( str, offset, err_list ) );
            case 'n' : return std::move( parse_null( str, offset, err_list ) );
...
JSON JSON::Load( const string &str, std::vector<std::string> * const err_list ) {
    size_t offset = 0;
    return std::move( parse_next( str, offset, err_list ) );
}
